...
import type { ParsedInput, Person } from "@/lib/types";
...
- 입력에 이름이 없고 기존 데이터도 없으면 name: "알 수 없음"으로.
...
export async function POST(req: Request) {
  try {
    ...
    const existing = body.existing
      ? JSON.stringify({
          name: body.existing.name,
          title: body.existing.title,
          company: body.existing.company,
          location: body.existing.location,
          education: body.existing.education,
          relationship: body.existing.relationship,
          family: body.existing.family,
          interests: body.existing.interests,
          business: body.existing.business,
          tags: body.existing.tags,
          notes: body.existing.notes,
        })
      : "null";
    ...
    if (!toolUse || toolUse.type !== "tool_use") {
      return NextResponse.json(
        { error: "AI did not return structured data" },
        { status: 502 }
      );
    }

    const parsed = toolUse.input as ParsedInput;
